module fc1 (
    ...
    // 输入流接口
    input  wire signed [7:0]   data_in_tdata,       // 串行输入 484 个 int8 (有符号)
    input  wire         data_in_tvalid,        // 输入完毕
    output wire         data_in_tready,        // 准备接收

    output reg  [8:0]   weight_addr,     // 矩阵列地址 (0-483，需要 9 bits)
    input  wire [471:0] weight_data_col, // 读到的整列权重 (118 * 4bit = 472 bits)

    // 输出数据流 (发往激活函数或下一层)
    output wire signed [7:0]   data_out_tdata,      // 串行输出 118 个 int8 (有符号)
    output wire         data_out_tvalid,       // 输出准备完毕
    input  wire         data_out_tready        // 接收端准备接收
);
    ...
    assign data_in_tready = (state == IDLE) || (state == CALC);
    ...
    always @(posedge clk or posedge rst) begin
        if (rst)
            ...
            in_cnt <= 9'd0;         // 空闲时置零
        else if (state == CALC && data_in_tvalid) begin
            if (in_cnt == 9'd483)
    ...
    always @(posedge clk or posedge rst) begin
        if (rst)
            ...
            out_cnt <= 7'd0;        // 空闲时置零
        else if (state == WRITE_BACK && data_out_tready) begin
            if (out_cnt == 7'd117)
    ...
    always @(posedge clk or posedge rst) begin
        ...
        else if (state == CALC && data_in_tvalid)
    ...
    always @(posedge clk or posedge rst) begin
        if (rst)
            ...
            data_in_reg <= data_in_tdata;  // 存储当前输入的向量元素
    end
